...
#if UW_LOG_LEVEL <= UW_DEBUG
#define p_debug(fmt, ...) printf("DEBUG: " fmt, ##__VA_ARGS__)
#else
#define p_debug(fmt, ...)
#endif

#if UW_LOG_LEVEL <= UW_INFO
#define p_info(fmt, ...) printf("INFO: " fmt, ##__VA_ARGS__)
#else
#define p_info(fmt, ...)
#endif

#if UW_LOG_LEVEL <= UW_ERROR
#define p_err(fmt, ...) printf("ERROR: " fmt, ##__VA_ARGS__)
#else
#define p_err(fmt, ...)
#endif

//eslee comment: not thread-safe, for api simplicity, similar bpf_lookup_elem
static struct sample_data *g_sample;
static size_t sample_stack_size;
static struct bpf_object *bpf_obj;
#if !defined(PTRACE_READ_MEMORY_FOR_REMOTE_UNWIND)
static int mem_fd = -1;

static int read_mem_init_proc(pid_t pid)
{
	char mem_path[256];
	...
	if (mem_fd == -1) {
		p_err("Failed to open %s: %s\n", mem_path, strerror(errno));
		return -EINVAL;
	}
	p_info("Accessing target memory via /proc filesystem\n");
	return 0;
}

static int read_mem_proc(pid_t pid, unw_word_t addr, unw_word_t *val)
{
	ssize_t len;
	off_t off;

	off = lseek(mem_fd, addr, SEEK_SET);
	if (off == (off_t)-1)
		return -UNW_EINVAL;
...
static void read_mem_deinit_proc(pid_t pid)
{
	if (mem_fd != -1) {
		close(mem_fd);
		mem_fd = -1;
...
#else
static int read_mem_init_ptrace(pid_t pid)
{
	int err = 0;

	if (ptrace(PTRACE_ATTACH, pid, 0, 0) != 0) {
		err = errno;
		p_err("Cannot attach to %d: %s\n", pid, strerror(errno));
		return -err;
	}
	p_info("Accessing target memory via ptrace\n");

	if (waitpid(pid, NULL, 0) == -1) {
		err = errno;
		p_err("Failed to waitpid: %s", strerror(errno));
		(void) ptrace(PTRACE_DETACH, pid, 0, 0);
		return -err;
	}
	p_info("Attached to pid %d\n", pid);

	return 0;
}

static int read_mem_ptrace(pid_t pid, unw_word_t addr, unw_word_t *val)
{
	int i, end;
	unw_word_t tmp_val;
...
static void read_mem_deinit_ptrace(pid_t pid)
{
	(void)ptrace(PTRACE_DETACH, pid, 0, 0);
	p_info("Detached from pid %d\n", pid);
}
#endif

#ifdef PTRACE_READ_MEMORY_FOR_REMOTE_UNWIND
#define read_mem_init read_mem_init_ptrace
#define read_mem read_mem_ptrace
#define read_mem_deinit read_mem_deinit_ptrace
#else
#define read_mem_init read_mem_init_proc
#define read_mem read_mem_proc
#define read_mem_deinit read_mem_deinit_proc
#endif
		      ...
		      int __write, void *arg)
{
	...
	*valp = *(unw_word_t *)&stack->data[offset];
	p_debug("unwind: start: %lx, end: %lx, addr: %p, offset: %x\n",
		       ...
		       unsigned long *ip, int ip_count)
{
	...
	err = unw_init_remote(&cursor, as, context);
	if (err < 0) {
		p_err("Cannot initialize cursor\n");
		goto cleanup;
	}
	...
	do {
		...
		if (err < 0) {
			p_err("Cannot read program counter\n");
			break;
		}
...
cleanup:
	p_info("Unwound %d stack frames, stopped by %d (%s)\n", i, err, unw_strerror(err));
	if (context)
...
static void print_dumped_stack(stack_dump_t *user_stack)
{
	...
	p_debug("## Dumped Stack (size: %d bytes) ##:\n", user_stack->size);
	for (int i = 0; i < nr; i++)
		p_debug("[%d]: %lx\n", i, ((unw_word_t*)user_stack->data)[i]);
	p_debug("\n");
}
		       ...
		       unsigned long *ip, size_t ip_count)
{
	...
	if (!stack_id || pid < 0 || !ip || ip_count == 0)
		return -EINVAL;
	...
	p_info("Dwarf unwind for stackID %d\n", *stack_id);

	err = get_entries(sample, pid, ip, ip_count);
	if (err)
...
int uw_map__set(const struct bpf_object *obj, size_t user_stack_size, size_t max_entries)
{
	...
	if (!obj || !samples_map || !ustacks_map)
		return -EINVAL;

	/* set max entries of bpf sample map */
	err = bpf_map__set_max_entries(samples_map, max_entries);
	if (err)
		return -err;

	/* set value size and max entries of bpf ustack map */
	err = bpf_map__set_max_entries(ustacks_map, max_entries);
	if (err)
		return -err;

	err = bpf_map__set_value_size(ustacks_map, user_stack_size);
	if (err)
		return -err;

	sample_stack_size = user_stack_size;
